...
import { ImageFormat, getAvailableFormats, getAvailableTargets } from "../utils/conversions";
...
export default function UnifiedConverter() {
  ...
  const ts = formatSelectorText[locale];

  const [from, setFrom] = useState<FromFormat>("detectar");
  const [to, setTo] = useState<ImageFormat>("jpg");
  const formats = getAvailableFormats();
  const targets = from === "detectar" ? formats : getAvailableTargets(from);
  const acceptForFrom = from === "detectar" ? "image/*" : MIME[from];

  const handleFromChange = (value: FromFormat) => {
    setFrom(value);
    const options = value === "detectar" ? formats : getAvailableTargets(value);
    if (!options.includes(to)) setTo(options[0]);
  };
  ...
  const convertAll = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        const blob = await convertImage(item.file, to);
  ...
  const handleDownload = (item: ManagedFile) => {
    ...
    if (!r?.result) return;
    downloadBlob(r.result, replaceExtension(item.file.name, to));
  };
  ...
  const handleDownloadZip = async () => {
    ...
    try {
      ...
      files.forEach((item) => {
        const r = results[item.id];
        if (!r?.result) return;
        const name = uniqueZipName(replaceExtension(item.file.name, to), usedNames);
        zip.file(name, r.result);
      });

      const zipBlob = await zip.generateAsync({ type: "blob" });
      downloadBlob(zipBlob, `imagens-convertidas-${to}.zip`);
    } catch {
              ...
              >
                {isConverting
                  ? files.length > 1
                    ...
                    : t.converting
                  : files.length > 1
                  ? t.convertMultipleButton(files.length, to.toUpperCase())
                  : t.convertSingleButton(to.toUpperCase())}
              </button>
            )}
